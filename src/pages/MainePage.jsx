...
import { getAllPosts, getSearchPosts } from "../redux/post/operatins";
...
useEffect(() => {
  getQuery !== "" ? dispatch(getSearchPosts(getQuery)) : dispatch(getAllPosts({getPage}))
}, [dispatch, getPage, getQuery])

  return (
    <div className="w-full md:max-w-[950px] mx-auto py-5 flex gap-5 flex-col">
        <PostsSearch setQuery={setQuery} getQuery={getQuery}/>
      <div className="flex justify-between md:gap-8 w-full">
        {loading ? <ThreeDots/> : posts?.length > 0 ? <PostsList posts={posts} setQuery={setQuery} getQuery={getQuery} totalPages={totalPages} currentPage={currentPage - 1}/> : <h2 className="m-auto">Наразі постів не знайдено...</h2>}
